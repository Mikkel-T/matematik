<script lang="ts">
  ...
  import { frac, na, numMulVec, pmat, vecLen } from "@math/calculation";

  function calculate() {
    let { a_1, a_2, a_3 } = calculator.opValues;
    answer.add(
      new Answer(
        "\\vec{e_a}",
        numMulVec(frac(na(1), vecLen({ x: a_1, y: a_2, z: a_3, name: "a" })), {
          x: a_1,
          y: a_2,
          z: a_3,
          name: "a",
...
<Calculator {calculate}>
  <Inputs inputs={[{ name: "a", type: "vec3d" }]} />
</Calculator>
